...
try:
        from modules.rng import randn_like, rand_like
except ImportError:
...
class CADSExtensionScript(scripts.Script):
        ...
        def ui(self, is_img2img):
                with gr.Accordion('CADS', open=False):
                        ...
                        with gr.Accordion('Schedule Function', open=False):
                                with gr.Row():
                                        schedule_fn = gr.Dropdown(value='Linear', choices=['Linear', 'Sigmoid 1', 'Sigmoid 2', 'Exponential', 'Absolute'], label="Schedule", elem_id='cads_schedule', info="Choose the annealing schedule function")
                        with gr.Row():
                ...
                schedule_fn.do_not_save_to_config = True
                ...
                return [active, t1, t2, noise_scale, mixing_factor, rescale, apply_to_hr_pass, schedule_fn]

        def before_process_batch(self, p, active, t1, t2, noise_scale, mixing_factor, rescale, apply_to_hr_pass, schedule_fn ,*args, **kwargs):
                ...
                schedule_fn = getattr(p, "cads_schedule", schedule_fn)
                ...
                p.extra_generation_params.update({
                        ...
                        "CADS Schedule": schedule_fn,
                })
                self.create_hook(p, active, t1, t2, noise_scale, mixing_factor, rescale, first_pass_steps, schedule_fn)
        
        def create_hook(self, p, active, t1, t2, noise_scale, mixing_factor, rescale, total_sampling_steps, schedule_fn, *args, **kwargs):
                # Use lambda to call the callback function with the parameters to avoid global variables
                y = lambda params: self.on_cfg_denoiser_callback(params, t1=t1, t2=t2, noise_scale=noise_scale, mixing_factor=mixing_factor, rescale=rescale, total_sampling_steps=total_sampling_steps, schedule_fn=schedule_fn)
        ...
        def ours_sigmoid_1_schedule(self, t, tau1, tau2, k=1.0):
        ...
        def ours_sigmoid_2_schedule(self, t, tau1, tau2, k=0.0):
                """CADS sigmoid annealing schedule function"""
                if t <= tau1:
                        return 1.0
                if t >= tau2:
        ...
        def cads_exponential_schedule(self, t, tau1, tau2, a=10.0):
        ...
        def cads_absolute_schedule(self, t, tau1, tau2, midpoint= 0.75):
        ...
        def add_noise(self, y, gamma, noise_scale, psi, rescale=False):
                ...
                print(">>>>>>>>>>>>>>>>> uniform noise 2")
                y = np.sqrt(gamma) * y + noise_scale * np.sqrt(1-gamma) * rand_like(y)
        ...
        def on_cfg_denoiser_callback(self, params: CFGDenoiserParams, t1, t2, noise_scale, mixing_factor, rescale, total_sampling_steps, schedule_fn):
                ...
                schedule_functions = {
                        'Linear': self.cads_linear_schedule,
                        'Sigmoid 1': self.ours_sigmoid_1_schedule,
                        'Sigmoid 2': self.ours_sigmoid_2_schedule,
                        'Exponential': self.cads_exponential_schedule,
                        'Absolute': self.cads_absolute_schedule,
                }

                # Select and call the appropriate schedule function based on the user's choice
                if schedule_fn in schedule_functions:
                        gamma_function = schedule_functions[schedule_fn]
                        gamma = gamma_function(t, t1, t2)
                        print(f"Using {schedule_fn} schedule function")
                else:
                        print("Unknown schedule function, using default linear schedule")
                        gamma = self.cads_linear_schedule(t, t1, t2)
